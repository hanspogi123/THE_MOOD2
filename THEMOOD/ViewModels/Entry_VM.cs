    ...
    public partial class Entry_VM : ObservableObject
    {
        ...
        private async Task LoginAsync()
        {
            if (!string.IsNullOrEmpty(Email) && !string.IsNullOrEmpty(Password))
            {
                ...
                // After navigating to main, navigate to Chat view
                await Task.Delay(100); // Small delay to ensure main page is loaded
                var navVM = NavBarViewModel.Instance;
                await navVM.NavigateToChatCommand.ExecuteAsync(null);
            }
            else
            {
